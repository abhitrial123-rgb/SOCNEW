...
from app.services.ingestion_scheduler import toggle_scheduler
...
def toggle(enabled: bool, user=Depends(get_current_user)):
    enforce_role(user, Role.manager)
    return {"enabled": toggle_scheduler(enabled)}
